    ...
    async def run(
        ...
        *,
        ...
        async with VastClient(self.config.api_key) as client:
            # --- BUNDLING PATCH ---
            num_gpus = self.spec.hardware.num_gpus
            if num_gpus > 1 and len(arm_pairs) > 1 and len(arm_pairs) <= num_gpus:
                msg = f"Bundling {len(arm_pairs)} arms onto a single {num_gpus}x GPU instance!"
                print(msg)
                ...
                boot_timeout = self.spec.budget.boot_timeout_s
                ssh_host, ssh_port = await self._wait_for_boot(
                    client, instance_id, boot_timeout,
                )
                if not ssh_host:
                ...
                runner = VastRunner(
                    client=client,
                    ssh_key_path=self.config.ssh_key,
                ...
                remote_path = self.spec.repo.remote_path
                train_timeout = self.spec.budget.train_timeout_s
                res = await runner.remote.run(
                    f"cd {remote_path} && {bundled_cmd}",
